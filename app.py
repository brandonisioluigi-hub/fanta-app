...
def carica_dati():
  ...
  # Gestione file nuovi acquisti se presente
  ...
  # Se ci sono nuovi acquisti, possiamo unirli o aggiornarli tramite ID se necessario
  # (Se hanno statistiche mancanti per il 25/26, impostiamo valori di default sicuri)
  for col in [
      'Presenze 25/26',
      'MV 25/26',
      'FM 25/26',
...
if giocatore_scelto:
  ...
  with col1:
    st.subheader('📊 Stagione Precedente / Statistiche')
    st.write(f"**Presenze:** {dati.get('Presenze 25/26', 'N.D.')}")
    st.write(f"**Media Voto (MV):** {dati.get('MV 25/26', 'N.D.')}")
    st.write(f"**Fantamedia (FM):** {dati.get('FM 25/26', 'N.D.')}")
    st.write(
        f"**Gol Fatti:** {dati.get('Gol 25/26', 'N.D.')} | **Assist:**"
        f" {dati.get('Assist 25/26', 'N.D.')}"
    )

    if dati['Ruolo'] == 'POR':
      st.write(f"**Gol Subiti:** {dati.get('Gs', 0)}")

    st.write(f"**Ammonizioni:** {dati.get('Ammoniz. 25/26', 'N.D.')}")
