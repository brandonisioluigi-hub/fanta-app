...
def carica_dati():
  ...
  # Estrazione rigorosa ed esaustiva dei prezzi da TUTTI i fogli completi del file Totale.xlsx
    ...
    try:
      ...
      for sheet in xls_tot.sheet_names:
        if 'completo' in sheet.lower() or 'statistiche' in sheet.lower():
          ...
          # Cerchiamo la riga di intestazione che contiene "Nome Giocatore"
          header_indices = df_s[
              df_s.astype(str)
          ...
          for idx in header_indices:
            ...
            # Puliamo le righe vuote o di separazione
            ...
            if valid_rows:
              ...
              # Prendiamo le prime 5 colonne (Nome, Frequenza, Max, Medio, Min) indipendentemente dai nomi esatti
  ...
  # Partiamo dal listone ufficiale (Quotazioni) per non perdere nessun giocatore
  merged = df_2627.copy()
  if 'Nome' in merged.columns and 'Nome Giocatore' not in merged.columns:
    merged['Nome Giocatore'] = merged['Nome']

  # Uniamo storico unificato e totale incrociato
  ...
  # Uniamo i prezzi prelevati unicamente da Totale.xlsx
...
if not df.empty and 'Nome Giocatore' in df.columns:
  ...
  if giocatore_scelto:
    ...
    # Prezzi presi ESCLUSIVAMENTE dal file Totale
