...
def carica_dati():
  ...
  # LA SVOLTA: Partiamo da df_2627 (Quotazioni) che contiene TUTTI i giocatori della Serie A
  merged = df_2627.copy()

  # Uniformiamo il nome del campo principale per l'interfaccia
  ...
  # Agganciamo i prezzi minimi calcolati
...
if not df.empty and 'Nome Giocatore' in df.columns:
  ...
  if giocatore_scelto:
    ...
    col_a1.metric(
        'Prezzo Max', f"{dati.get('Prezzo Max', 0) if pd.notna(dati.get('Prezzo Max')) else 0} cr."
    )
