...
@st.cache_data
def carica_dati():
  # Riconoscimento dinamico dei 3 file puliti
  ...
  # Estrazione Prezzi Minimi dal file Totale
    ...
    try:
      ...
      for sheet in [
        ...
        if sheet in xls_tot.sheet_names:
          df_s = pd.read_excel(file_tot, sheet_name=sheet)
  ...
  # Merge pulito basato su Nome (Totale + Quotazioni) e ID (Statistiche unificate)
  merged = (
      pd.merge(
          df_tot,
  ...
  if not merged.empty and not df_prices.empty:
    merged = pd.merge(merged, df_prices, on='Nome_Clean', how='left')

  if not df_2526.empty and not merged.empty and 'Id' in df_2526.columns:
...
if not df.empty and 'Nome Giocatore' in df.columns:
  ...
  if giocatore_scelto:
    ...
    col_a3.metric(
        'Prezzo Medio',
        f"{int(round(float(p_medio)))} cr." if pd.notna(p_medio) else '0 cr.',
    )
  ...
  st.error(
      'Errore nel caricamento dei dati. Assicurati che i file Excel unificati'
      ' siano presenti nella cartella.'
  )