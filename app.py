...
def carica_dati():
  ...
  # Merge con i nuovi acquisti (prioritario per chi non ha lo storico)
  if not df_nuovi.empty and not merged.empty and 'Id_Key' in df_nuovi.columns:
    merged = pd.merge(
        merged,
        df_nuovi,
        on='Id_Key',
...
if not df.empty and 'Nome Giocatore' in df.columns:
  ...
  if giocatore_scelto:
    ...
    # Funzione intelligente che unisce i campi prendendo prioritariamente dal file nuovi acquisti o dallo storico
    def get_stat(*col_names):
      for c in col_names:
        if c in dati and pd.notna(dati[c]):
          val = dati[c]
          # Se il valore è valido e diverso da zero (o se è l'unica opzione)
          if val != 0 and val != '0' and val != 'N.D.' and val != '':
            return val
      # Se c'è comunque un campo numerico salvato come 0 ma esiste, restituiamo quello o N.D.
      for c in col_names:
        if c in dati and pd.notna(dati[c]):
          return dati[c]
      return 0
    ...
    col_a4.metric('FVM Consigliato', f"{dati.get('FVM 27', dati.get('FVM 26/27', 'N.D.'))}")

    st.divider()
    col1, col2 = st.columns(2)
    with col1:
      st.subheader('📊 Statistiche / Storico')
      # Cerchiamo prima nelle colonne dei nuovi acquisti, poi nello storico standard
      presenze = get_stat('Presenze_nuovi', 'Presenze', 'Presenze 25/26', 'P')
      mv = get_stat('MV_nuovi', 'MV', 'MV 25/26')
      fm = get_stat('FM_nuovi', 'FM', 'FM 25/26')
      gol = get_stat('Gol_nuovi', 'Gol', 'Gol 25/26', 'Gf')
      assist = get_stat('Assist_nuovi', 'Assist', 'Assist 25/26', 'Ass')
      ammoniz = get_stat('Ammoniz._nuovi', 'Ammoniz.', 'Ammoniz. 25/26', 'Au')
