...
if not df.empty and 'Nome Giocatore' in df.columns:
  ...
  if giocatore_scelto:
    ...
    # Funzione di supporto flessibile per recuperare la statistica da qualsiasi colonna disponibile
    def get_stat(*keys):
      for k in keys:
        if k in dati and pd.notna(dati[k]):
          val = dati[k]
          if val != 0 and val != '0' and val != 'N.D.':
            return val
      return 0
    ...
    with col1:
      ...
      presenze = get_stat('Presenze 25/26', 'Presenze', 'Presenze_nuovi', 'P')
      mv = get_stat('MV 25/26', 'MV', 'MV_nuovi')
      fm = get_stat('FM 25/26', 'FM', 'FM_nuovi')
      gol = get_stat('Gol 25/26', 'Gol', 'Gol_nuovi', 'Gf')
      assist = get_stat('Assist 25/26', 'Assist', 'Assist_nuovi', 'Ass')
      ammoniz = get_stat('Ammoniz. 25/26', 'Ammoniz.', 'Ammoniz._nuovi', 'Au')
