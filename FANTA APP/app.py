...
# 1. Caricamento e Unione dei file con calcolo di prezzi interi e classifiche di frequenza
@st.cache_data
def carica_dati():
    df_2526 = pd.read_excel('Statistiche_Fantacalcio_Stagione_2025_26.xlsx', sheet_name='Tutti', header=1)
    df_2627 = pd.read_excel('Quotazioni_Fantacalcio_Stagione_2026_27 (1).xlsx', sheet_name='Tutti', header=1)
    df_tot = pd.read_excel('Totale.xlsx', sheet_name='Statistiche_Incrociate')
    
    # Estraiamo anche il prezzo minimo dai fogli completi per ruolo se disponibile
    xls_tot = pd.ExcelFile('Totale.xlsx')
    complete_sheets = ['Portieri_Completo', 'Difensori_Completo', 'Centrocampo_Completo', 'Attacco_Completo']
    all_role_sheets = []
    for sheet in complete_sheets:
        ...
        for idx in header_indices:
            ...
            for _, row in sub.iterrows():
                val0 = str(row.iloc[0])
                if pd.isna(row.iloc[0]) or 'TOP' in val0.upper() or 'MEDI' in val0.upper() or 'LOW' in val0.upper() or '🌟' in val0:
                    break
                valid_rows.append(row)
            if valid_rows:
                sub_clean = pd.DataFrame(valid_rows)
                sub_clean.columns = ['Nome Giocatore', 'Frequenza', 'Prezzo Max', 'Prezzo Medio', 'Prezzo Min']
                all_role_sheets.append(sub_clean)
                
    if all_role_sheets:
        df_prices = pd.concat(all_role_sheets, ignore_index=True)
        df_prices['Nome_Clean'] = df_prices['Nome Giocatore'].astype(str).str.strip().str.lower()
        df_prices_unique = df_prices.groupby('Nome_Clean')['Prezzo Min'].min().reset_index()
    else:
        df_prices_unique = pd.DataFrame(columns=['Nome_Clean', 'Prezzo Min'])

    # Pulizia e uniformazione nomi
    df_tot['Nome_Clean'] = df_tot['Nome Giocatore'].astype(str).str.strip().str.lower()
    ...
    merged = pd.merge(merged, df_2526, on='Id', how='left', suffixes=('', '_2526_file'))
    
    # Arrotondamento prezzo medio senza decimali
    merged['Prezzo Medio Int'] = merged['Prezzo Medio'].round().astype(int)
    
    # Calcolo della classifica basata sulla frequenza (dalla più alta alla più bassa)
    merged['Rank_Freq'] = merged['Freq. Asta'].rank(ascending=False, method='min').astype(int)
...
def carica_ranking(sheet_name):
    ...
    for _, row in df.iterrows():
        ...
        if "AFFIDABILITÀ" in val0.upper() or "PRESTAZIONI" in val0.upper():
            if "POR" in val0.upper(): current_role = "POR"
            elif "DIF" in val0.upper(): current_role = "DIF"
            elif "CEN" in val0.upper(): current_role = "CEN"
            elif "ATT" in val0.upper(): current_role = "ATT"
            rank_in_role = 0
            continue
...
st.markdown("Cerca un giocatore per analizzare lo storico, i prezzi d'asta e la posizione in classifica.")
...
if giocatore_scelto:
    ...
    st.write(f"**Ruolo:** {dati['Ruolo']} | **Squadra 26/27:** {dati['Squadra 26/27']}")
    st.divider()
    
    # Sezione Prezzi e Frequenza con Classifica
    st.info("📋 **Dati Simulazioni & Prezzi Asta**")
    
    col_a1, col_a2, col_a3, col_a4, col_a5 = st.columns(5)
    
    # Mostriamo frequenza e la sua posizione in classifica
    ...
    # Gestione prezzo min (se presente o calcolato)
    ...
    with col2:
        ...
        qt_a = dati.get('Qt.A')
        qt_i = dati.get('Qt.I')
        st.write(f"**Quotazione Iniziale (Qt.I):** {qt_i if pd.notna(qt_i) else 'N.D.'}")
        st.write(f"**Quotazione Attuale (Qt.A):** {qt_a if pd.notna(qt_a) else 'N.D.'}")
